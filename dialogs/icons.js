...
CKEDITOR.dialog.add( 'icons', function( editor ) {
	var config = editor.config,
		lang = editor.lang.icons,
		images = config.icons_images,
		columns = config.icons_columns || 8,
		i;
	...
	for ( i = 0; i < size; i++ ) {
		...
		html.push( '<td class="cke_dark_background cke_centered" style="vertical-align: middle;" role="presentation">' +
			'<a href="javascript:void(0)" role="option"', ' aria-posinset="' + ( i + 1 ) + '"', ' aria-setsize="' + size + '"', ' aria-labelledby="' + smileyLabelId + '"', ' class="cke_smile cke_hand" tabindex="-1" onkeydown="CKEDITOR.tools.callFunction( ', onKeydown, ', event, this );">', '<img class="cke_hand" title="', config.icons_descriptions[ i ], '"' +
					' cke_src="', CKEDITOR.tools.htmlEncode( config.icons_path + images[ i ] ), '" alt="', config.icons_descriptions[ i ], '"', ' src="', CKEDITOR.tools.htmlEncode( config.icons_path + images[ i ] ), '"',
		// IE BUG: Below is a workaround to an IE image loading bug to ensure the image sizes are correct.
		( CKEDITOR.env.ie ? ' onload="this.setAttribute(\'width\', 2); this.removeAttribute(\'width\');" ' : '' ), '>' +
				'<span id="' + smileyLabelId + '" class="cke_voice_label">' + config.icons_descriptions[ i ] + '</span>' +
			'</a>', '</td>' );
